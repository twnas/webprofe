...
$('#generarReporteButton').click(function () {
  let idGrado = $('#gradoSelect').val();
  let idArea = $('#areaSelect').val();
  console.log("Reporte", idGrado, idArea)
  if (idGrado && idArea) {
    fetch(`/api/gradosSecciones/historialLogrosGrado/${idGrado}/${idArea}`)
      .then((response) => {
        return response.json();
      })
      .then((secciones) => {
        ...
        $('#seccionesLista').html('Secciones: '+seccionesCadena);
